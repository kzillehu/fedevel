...
\TLV
   ...
   |cpu
      ...
      @1
         ...
         ?$rd_valid
            ...
            //updating rd for load and rf-writes
            //$rf_wr_index[4:0] = $rd;
         ...
         //extend decode for other RISC-V instructions
      ...
      @2 
         ...
         //moving write enable to stage 3 to match reference solution
         //$rf_wr_en = >>1$valid && $rd_valid && !($rd ==? 5'b0);
         
         $src1_value[31:0] = (>>1$rf_wr_en && (>>1$rd == $rs1)) ? >>1$result : $rf_rd_data1; 
         $src2_value[31:0] = (>>1$rf_wr_en && (>>1$rd == $rs2)) ? >>1$result : $rf_rd_data2; 
         
         $br_tgt_pc[31:0] = $pc + $imm; //this is also used by JAL
         $jalr_tgt_pc = $src1_value + $imm;
      @3
         //updating enable writes to rf for valid loads
         ...
         //updating valid in face of load instructions
         $valid = !(>>1$valid_taken_br || >>2$valid_taken_br || >>1$valid_load || >>2$valid_load || >>1$valid_jump || >>2$valid_jump);

         //adding mux for write rf
      ...
      @4
         ...
         `BOGUS_USE($is_jump $is_beq $is_bne $is_blt $is_bge $is_bltu $is_bgeu $is_addi $taken_br)
   ...
   //*passed = *cyc_cnt > 40;
   *passed = |cpu/xreg[15]>>5$value == (1+2+3+4+5+6+7+8+9);	//just go forward a couple of cycles
   *failed = 1'b0;
